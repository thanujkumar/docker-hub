def dockerImage;
//docker node is configured in jenkins /configureClouds 
node('docker'){
	stage('SCM'){
	...
	stage('build'){
		dockerImage = docker.build('thanujtk/jenkins:lts-jdk11-dotnet-v$BUILD_NUMBER', './pluralsight_jenkins/jenkins-docker/meta-build/dotnet-image');
	}
	stage('push'){
